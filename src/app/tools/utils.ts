...
export class util {
  ...
  get_typecontact(type){
    var typeContact =[{val:1,status:false},{val:2,status:false}] ;
    if (type==1|| type ==3){
      typeContact[0].status=true
    }
    if(type==2|| type ==3){
      typeContact[1].status=true
    }
    return typeContact
    }
    set_typecontact(typeContact){
      var type = 0 ;
      if (  typeContact[0].status==true ){
        type = 1
      }
      if(typeContact[1].status==true){
        type = type + 2
      }
      return type
      }
